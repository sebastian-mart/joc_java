...
public class ForestGoblin extends NPC{
    private static final float DETECTION_RANGE = 200.0f;
    private static final float ATTACK_RANGE = 75.0f;
    ...
    public ForestGoblin(RefLinks refLink, float x, float y) {
        super(refLink, x, y);
        image = Assets.GoblinWalkLeft;
        isSpawned = true;
        speed = 3.5f;
        spawnX = (int) x;
    ...
    private void UpdateAttackAnimation(){
        if (isAttacking) {
            ...
            if (animationTimer >= ATTACK_ANIMATION_SPEED) {
                ...
                if (xMove < 0) { // Merge spre stânga
                    if (currentAttackFrame == 0) {
                        image = Assets.GoblinAttack1Left;
                    } else {
                        image = Assets.GoblinAttack2Left;
                    }
                } else { // Merge spre dreapta
                    if (currentAttackFrame == 0) {
                        image = Assets.GoblinAttack1Right;
                    } else {
                        image = Assets.GoblinAttack2Right;
                    }
                }
            }
        } else {
            ...
            image = (xMove<0)? Assets.GoblinWalkLeft : Assets.GoblinWalkRight;
        }
    }
    ...
    public void Move() {
        ...
        if (horizontal_distance < ATTACK_RANGE && vertical_distance < VERTICAL_ATTACK_THRESHOLD) {
            ...
            image = (x < heroX) ? Assets.GoblinAttack1Right : Assets.GoblinAttack1Left;
        }
    ...
    protected void PerformAttack() {
        Hero hero = refLink.GetHero();
        if (hero == null) return;
        System.out.println("A atacat");
        float distance = Math.abs(hero.GetX() - x);
        if (distance < ATTACK_RANGE) {
            hero.TakeDamage(5); // Presupunem că Hero are o metodă TakeDamage
